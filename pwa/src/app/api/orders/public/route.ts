...
export async function GET() {
  ...
  try {
    ...
    if (error) {
      log.error('GET /api/orders/public', { error: String(error), code: (error as any)?.code, details: (error as any)?.details });
    ...
    log.error('GET /api/orders/public', { error: String(err) });
    return NextResponse.json({ ok: true, orders: [] });
  }
}
