...
export default function TendersPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8">
      <h1 className="text-2xl font-semibold text-zinc-900">Тендеры</h1>
      <p className="mt-2 text-sm text-zinc-600">
        Поиск по базе (нужен{' '}
        <code className="rounded bg-zinc-100 px-1">NEXT_PUBLIC_TENDER_API_URL</code> на FastAPI).
      </p>

      {stats && (
        <section className="mt-6 grid gap-2 rounded-lg border border-zinc-200 bg-white p-4 text-sm">
          <div>Всего в выборке: {String((stats as { total?: number }).total ?? '—')}</div>
        </section>
      )}

      <section className="mt-6 space-y-3 rounded-lg border border-zinc-200 bg-white p-4">
      ...
      <section className="mt-6 overflow-x-auto rounded-lg border border-zinc-200 bg-white">
        ...
        <div className="flex items-center justify-between px-3 py-2 text-sm">
          ...
          <span>Стр. {page}</span>
          <button type="button" className="rounded border border-zinc-300 px-3 py-1" onClick={() => setPage((p) => p + 1)}>
            Вперёд
          </button>
        </div>
      </section>

      <section className="mt-8 rounded-lg border border-dashed border-zinc-300 p-4 text-sm text-zinc-600">
        <p className="font-medium text-zinc-800">Telegram Login (виджет)</p>
        <p className="mt-1">
          Подключите виджет авторизации на этой странице и свяжите{' '}
          <code className="rounded bg-zinc-100 px-1">telegram_user_id</code> с сессией PWA по аналогии с
          существующим auth.
        </p>
      </section>
    </main>
