...
Корпоративная площадка закупок ГК «Росатом».
"""
...
class RosatomScraper(BaseScraper):
    ...
    def _parse_page(self, html: str) -> list[dict]:
        ...
        blocks = soup.select(
            ...
            "[class*='purchase'], [class*='tender']"
        )

        for block in blocks:
            item = {}
            link = block.select_one(
                "a[href*='/purchase/'], a[href*='/lot/'], a[href*='/procedure/'], "
        ...
        if not results:
            for a in soup.select("a[href*='/purchase/'], a[href*='/lot/']"):
