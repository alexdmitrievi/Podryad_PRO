...
Электронная торговая площадка ПАО «Газпром».
"""
...
class GazpromScraper(BaseScraper):
    ...
    def _parse_page(self, html: str) -> list[dict]:
        ...
        if not results:
            for a in soup.select("a[href*='/purchase'], a[href*='/procedure']"):
        ...
        with self:
            for query in queries:
                ...
                for page in range(1, max_pages + 1):
                    try:
                        url = f"{self.base_url}/rfx/search?{urlencode({'query': query, 'page': str(page)})}"
                        resp = self.fetch(url)
