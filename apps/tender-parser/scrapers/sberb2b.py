...
Корпоративная торговая площадка Сбера для закупок бизнеса.
"""
...
class SberB2BScraper(BaseScraper):
    ...
    def _parse_page(self, html: str) -> list[dict]:
        ...
        blocks = soup.select(
            ".purchase-item, .tender-card, .search-result-item, "
            "tr.resultRow, [data-purchase-id], article, .lot-item"
        )
        ...
        for block in blocks:
            ...
            customer_el = block.select_one(".customer, .organizer, .org-name")
            if customer_el:
                item["customer"] = customer_el.get_text(strip=True)

            date_el = block.select_one(".deadline, .date-end, .end-date, time")
            if date_el:
    ...
    def run(self, queries: list[str] | None = None, max_pages: int = 2, **kwargs) -> list[TenderCreate]:
        if queries is None:
            queries = [
                "поставка оборудования", "ремонт", "строительные работы",
        ...
        with self:
            for query in queries:
                ...
                for page in range(1, max_pages + 1):
                    try:
                        url = f"{self.base_url}/market/search?{urlencode({'query': query, 'page': str(page)})}"
                        resp = self.fetch(url)
